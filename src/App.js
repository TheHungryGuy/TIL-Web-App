...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  async function handleSubmit(e) {
    ...
    //2. Check if data is valid. If so, create a new valid fact
    if (text && isValidHttpUrl(source) && category && textLength <= 200) {
      ...
      //4. Add the new fact to the UT: add the fact to state
...
function FactList({ facts, setFacts }) {
  ...
  return (
    <section>
      ...
      <p>
        There are {facts.length} facts in the current database. Add your own!
      </p>
    </section>
  );
}
